    ...
    try {
      ...
      if (error.response) {
        if (error.response.status == 400) {
          setError('Ops, não foi possível incluir o usuário. Por favor, verifique o nome e o período e tente novamente.')
        } else {
    ...
    <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title" fullWidth={true} maxWidth="sm">
      <CustomDialogContent customtheme={theme}>
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          />
          {errors.email && (
            <p className="error">Preencha corretamente o e-mail</p>
          )}
          <Controller
            name="password"
            control={control}
            defaultValue=""
            as={<CustomInput type="password" label="Senha" variant="filled" required ref={register} autoComplete="" />}
          />
          {errors.password && (
          ...
          {errors.role && (
            <p className="error">Preencha corretamente o perfil</p>
          )}
          {!loading ? (
            <DialogActions>
